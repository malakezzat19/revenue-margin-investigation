...
Generates a realistic messy two-year dataset with these exact issues baked in (generate_data.py)
Cleans the data step by step: removes duplicates, standardizes discount format, imputes missing shipping cost by category, handles null return flags
Analyzes revenue and profit margin year-over-year
Identifies the real drivers: order volume growth, a shift toward a higher-discount acquisition channel (Paid Ads), a shift toward a lower-margin product category (Electronics), and rising shipping costs
Gives a specific recommendation: rebalance acquisition spend away from over-reliance on Paid Ads, and revisit Electronics pricing/costs
Key findings
